...
export class ReproductorYoutubeComponent implements OnInit {
  ...
  private load(): void {
    ...
    this.yt
      .getVideoDetailsById(this.videoId) // snippet + statistics + duration
      .pipe(
        switchMap((resp) => {
          ...
          return forkJoin({
            related: this.yt
              ...
              .pipe(catchError(() => of([]))),
            more: this.yt
              .getChannelVideosByName(this.channelName, 8)
              .pipe(catchError(() => of([]))),
            channel: this.yt
      ...
      .subscribe((bundle) => {
        ...
        if (ch) {
          ...
          this.subscriberCount = subs
            ? subs.toLocaleString('en-US') + ' subscriptors'
            : '';
        }
        ...
        const mapCreator = (v: YoutubeVideo): YoutubeVideo => ({
          ...
          /* si no viniera del servicio la generamos aquí */
